...
public:
  ...
  : Node(options.node_name)
  {
    ...
    for (size_t i = 0; i < options.topic_names_pub.size(); ++i) {
      ...
      end_time_pub_[topic_name] = start_time_pub_[topic_name] +
    ...
    // }
        ...
        // 単独なら、通常通りtimerでpub
      if (std::find(options.topic_names_sub.begin(), options.topic_names_sub.end(),
        topic_name) == options.topic_names_sub.end())
      {
        auto publish_message =
          [this, topic_name, payload_size, eval_time = options.eval_time]() -> void
          {
            ...
            message_->header.stamp.sec = static_cast<int32_t>(time_stamp.seconds() -
              start_time_pub_[topic_name].seconds());
            message_->header.stamp.nanosec = static_cast<uint32_t>((time_stamp.nanoseconds() -
        ...
        auto shutdown_timer = create_wall_timer(std::chrono::seconds(options.eval_time + 10),
          shutdown_node);
        shutdown_timers_.emplace(topic_name, shutdown_timer);
      }
    ...
    for (size_t i = 0; i < options.topic_names_sub.size(); ++i) {
      ...
      end_time_sub_[topic_name] = start_time_sub_[topic_name] +
        rclcpp::Duration::from_seconds(options.eval_time);
    ...
    // }
      ...
      std::function<void(const publisher_node::msg::IntMessage::SharedPtr)> callback;
        ...
        // 単独なら
      if (std::find(options.topic_names_pub.begin(), options.topic_names_pub.end(),
        topic_name) == options.topic_names_pub.end())
      {
        auto callback = [this, topic_name,
            eval_time =
            options.eval_time](const publisher_node::msg::IntMessage::SharedPtr message_) -> void
          {
            auto sub_time = this->get_clock()->now();
            if((sub_time.seconds() - start_time_sub_[topic_name].seconds()) >= eval_time) {
                    // RCLCPP_INFO(this->get_logger(), "Topic %s has reached the evaluation time.", topic_name.c_str());
        ...
        auto subscriber = create_subscription<publisher_node::msg::IntMessage>(topic_name, qos,
          callback);
        subscribers_.emplace(topic_name, subscriber);
        ...
        auto callback =
            ...
            options.eval_time](const publisher_node::msg::IntMessage::SharedPtr message_) -> void
          {
            ...
            message_->header.stamp.sec = static_cast<int32_t>(sub_time.seconds() -
              start_time_sub_[topic_name].seconds());
            message_->header.stamp.nanosec = static_cast<uint32_t>((sub_time.nanoseconds() -
              start_time_sub_[topic_name].nanoseconds()) % 1000000000);
            ...
            oss << std::dec << "Time: " << std::fixed << std::setprecision(9) <<
              static_cast<double>(pub_time.nanoseconds() -
            start_time_pub_[topic_name].nanoseconds()) / 1e9;
                // RCLCPP_INFO(this->get_logger(), "Publish/ Topic: %s Data: %s Index: %d", topic_name.c_str(), oss.str().c_str(), current_pub_idx);
...
private:
  std::unordered_map<std::string,
    rclcpp::Publisher<publisher_node::msg::IntMessage>::SharedPtr> publishers_;
  std::unordered_map<std::string,
    rclcpp::Subscription<publisher_node::msg::IntMessage>::SharedPtr> subscribers_;
  ...
  {
    std::filesystem::path p = std::filesystem::path(log_dir) / (options.node_name + "_log") /
      "metadata.txt";
  ...
  void record_log_pub_(
    const std::string & topic_name, const std::string & pub_node_name,
    const uint32_t & message_idx, const rclcpp::Time & time_stamp)
  {
    MessageLog log = {pub_node_name, message_idx, time_stamp};
    message_logs_pub_[topic_name].emplace_back(log);
  }
  void record_log_sub_(
    const std::string & topic_name, const std::string & pub_node_name,
    const uint32_t & message_idx, const rclcpp::Time & time_stamp)
  {
    MessageLog log = {pub_node_name, message_idx, time_stamp};
  ...
  void write_all_logs_pub_(const std::map<std::string, std::vector<MessageLog>> & message_logs_pub_)
  {
    for (const auto &[topic_name, topic_logs] : message_logs_pub_) {
      std::filesystem::path p = std::filesystem::path(log_dir) / (node_name + "_log") /
