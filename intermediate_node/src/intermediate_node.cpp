...
static node_options::Options parse_options(int argc, char** argv) {
  ...
  auto options =
      node_options::Options(non_ros_argc, non_ros_args_c_strings.data());
...
static void create_result_directory(const node_options::Options& options) {
  ...
  for (size_t i = 0; i < options.topic_names_pub.size(); ++i) {
    ss << result_dir_name << "/" << options.topic_names_pub[i]
       << "_pub_log.txt";
  ...
  for (size_t i = 0; i < options.topic_names_sub.size(); ++i) {
    ss << result_dir_name << "/" << options.topic_names_sub[i]
       << "_sub_log.txt";
    std::string log_file_path = ss.str();
    log_file_paths.push_back(log_file_path);
    ss.str("");
 ...
 public:
  explicit Intermediate(const node_options::Options& options)
    ...
    for (size_t i = 0; i < options.topic_names_pub.size(); ++i) {
      ...
      end_time_pub_[topic_name] =
          start_time_pub_[topic_name] +
          rclcpp::Duration::from_seconds(options.eval_time);
      ...
      //       [this, topic_name, payload_size, eval_time = options.eval_time]()
      //       -> void
      //     {
      //       int current_pub_idx = pub_idx_[topic_name];
      //       auto time_stamp = this->get_clock()->now();

      //       if ((time_stamp.seconds() -
      //       start_time_pub_[topic_name].seconds()) >= eval_time) {
      //         RCLCPP_INFO(this->get_logger(), "Topic %s has reached the
      //         evaluation time.", topic_name.c_str());
      ...
      //       auto message_ =
      //       std::make_shared<publisher_node::msg::IntMessage>();
      //       message_->data.resize(payload_size);
      //       std::fill(message_->data.begin(), message_->data.end(), 0);

      //       message_->header.stamp.sec =
      //       static_cast<int32_t>(time_stamp.seconds() -
      //       start_time_pub_[topic_name].seconds());
      //       message_->header.stamp.nanosec =
      //       static_cast<uint32_t>((time_stamp.nanoseconds() -
      //       start_time_pub_[topic_name].nanoseconds()) % 1000000000);
      //       message_->header.pub_idx = current_pub_idx;
      //       message_->header.node_name = node_name;

      //       record_log_pub_(topic_name, node_name, current_pub_idx,
      //       time_stamp);
      ...
      //           << static_cast<double>(time_stamp.nanoseconds() -
      //           start_time_pub_[topic_name].nanoseconds()) / 1e9;

      //       RCLCPP_INFO(this->get_logger(), "Publish/ Topic: %s, Data: %s,
      //       Index: %d", topic_name.c_str(), oss.str().c_str(),
      //       current_pub_idx); publishers_[topic_name]->publish(*message_);
      //       pub_idx_[topic_name]++;
      //     };
      ...
      if (std::find(options.topic_names_sub.begin(),
        ...
        auto publish_message = [this, topic_name, payload_size,
          ...
          auto time_stamp = this->get_clock()->now();
          if ((time_stamp.seconds() - start_time_pub_[topic_name].seconds()) >=
              eval_time) {
            // RCLCPP_INFO(this->get_logger(), "Topic %s has reached the
            // evaluation time.", topic_name.c_str());
            timers_[topic_name]->cancel();
          ...
          message_->header.stamp.sec = static_cast<int32_t>(
              time_stamp.seconds() - start_time_pub_[topic_name].seconds());
          message_->header.stamp.nanosec = static_cast<uint32_t>(
              (time_stamp.nanoseconds() -
               start_time_pub_[topic_name].nanoseconds()) %
              1000000000);
          message_->header.pub_idx = current_pub_idx;
          message_->header.node_name = node_name;
        ...
        auto shutdown_timer = create_wall_timer(
    ...
    for (size_t i = 0; i < options.topic_names_sub.size(); ++i) {
      ...
      //     auto callback =
      //     [this, topic_name, self_node = node_name, eval_time =
      //     options.eval_time](const publisher_node::msg::IntMessage::SharedPtr
      //     message_) -> void
      //   {
      ...
      //     eval_time) {
      //       RCLCPP_INFO(this->get_logger(), "Topic %s has reached the
      //       evaluation time.", topic_name.c_str()); end_time_sub_[topic_name]
      //       = sub_time; return;
      //     }
      ...
      //     }
      //     oss << std::dec << "Time: " << std::fixed << std::setprecision(9)
      //         << static_cast<double>(sub_time.nanoseconds() -
      //         start_time_sub_[topic_name].nanoseconds()) / 1e9;

      //     int current_pub_idx = message_->header.pub_idx;
      //     std::string pub_node_name = message_->header.node_name;
      //     RCLCPP_INFO(this->get_logger(), "Subscribe/ Topic: %s Data: %s
      //     Index: %d", topic_name.c_str(), oss.str().c_str(),
      //     current_pub_idx); record_log_sub_(topic_name, pub_node_name,
      //     current_pub_idx, sub_time);
      //   };

      //   auto subscriber =
      //   create_subscription<publisher_node::msg::IntMessage>(topic_name, qos,
      //   callback); subscribers_.emplace(topic_name, subscriber);
      ...
      if (std::find(options.topic_names_pub.begin(),
        ...
        auto callback =
          ...
          oss << std::dec << "Time: " << std::fixed << std::setprecision(9)
              << static_cast<double>(
                     sub_time.nanoseconds() -
                     start_time_sub_[topic_name].nanoseconds()) /
                     1e9;
          int current_pub_idx = message_->header.pub_idx;
          std::string pub_node_name = message_->header.node_name;
          // RCLCPP_INFO(this->get_logger(), "Subscribe/ Topic: %s Data: %s
      ...
      // 兼任なら
        ...
        auto callback =
          ...
          message_->header.stamp.sec = static_cast<int32_t>(
              sub_time.seconds() - start_time_sub_[topic_name].seconds());
          message_->header.stamp.nanosec = static_cast<uint32_t>(
              (sub_time.nanoseconds() -
               start_time_sub_[topic_name].nanoseconds()) %
              1000000000);
          ...
          if ((pub_time.seconds() - start_time_pub_[topic_name].seconds()) >=
              eval_time) {
            // RCLCPP_INFO(this->get_logger(), "Topic %s has reached the
            // evaluation time.", topic_name.c_str());
          ...
          oss << std::dec << "Time: " << std::fixed << std::setprecision(9)
              << static_cast<double>(
                     pub_time.nanoseconds() -
                     start_time_pub_[topic_name].nanoseconds()) /
                     1e9;
          // RCLCPP_INFO(this->get_logger(), "Publish/ Topic: %s Data: %s Index:
          // %d", topic_name.c_str(), oss.str().c_str(), current_pub_idx);
 ...
 private:
  ...
  void create_metadata_file(const node_options::Options& options) {
    ...
    if (ec) {
      RCLCPP_WARN(this->get_logger(),
                  "Failed to ensure metadata dir: %s (errno=%d: %s)",
                  p.parent_path().string().c_str(), errno,
                  std::strerror(errno));
    }
    ...
    file.close();
    RCLCPP_INFO(this->get_logger(), "Metadata written to file: %s",
                p.string().c_str());
  }
  ...
  void write_all_logs_pub_(
    ...
    for (const auto& [topic_name, topic_logs] : message_logs_pub_) {
      ...
      if (ec) {
        RCLCPP_WARN(this->get_logger(),
                    "Failed to ensure pub log dir: %s (errno=%d: %s)",
                    p.parent_path().string().c_str(), errno,
                    std::strerror(errno));
      }
      ...
      file.close();
      RCLCPP_INFO(this->get_logger(), "MessageLogs written to file: %s",
                  p.string().c_str());
    }
  }
  ...
  void write_all_logs_sub_(
    ...
    for (const auto& [topic_name, topic_logs] : message_logs_sub_) {
      ...
      if (ec) {
        RCLCPP_WARN(this->get_logger(),
                    "Failed to ensure sub log dir: %s (errno=%d: %s)",
                    p.parent_path().string().c_str(), errno,
                    std::strerror(errno));
      }
