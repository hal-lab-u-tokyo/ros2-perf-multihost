...
class Subscriber : public rclcpp::Node {
  ...
  explicit Subscriber(const node_options::Options &options)
    ...
    for (size_t i = 0; i < options.topic_names.size(); ++i) {
      ...
      auto callback =
        ...
        // RCLCPP_INFO(this->get_logger(), "Subscribe/ Topic: %s Data: %s Index:
 ...
 private:
  ...
  void record_log(const std::string &topic_name,
                  const std::string &pub_node_name, const uint32_t &message_idx,
