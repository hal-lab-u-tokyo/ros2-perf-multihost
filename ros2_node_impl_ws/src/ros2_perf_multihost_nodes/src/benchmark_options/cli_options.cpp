...
void Options::parse(int argc, char **argv) {
  ...
  options.add_options()("h,help", "Show this help message and exit")(
      ...
      "qos-override",
      "Apply global QoS options to every endpoint: true or false",
      cxxopts::value<std::string>(qos_override_value)->default_value("false"));
  ...
  try {
    ...
    if (qos_override_value != "true" && qos_override_value != "false") {
      std::cout << "Error: --qos-override must be true or false.\n\n";
      print_help();
      std::exit(1);
    }
    qos_override = qos_override_value == "true";
    ...
    if (qos_reliability != "RELIABLE" && qos_reliability != "BEST_EFFORT") {
      std::cout
          << "Error: --qos-reliability must be RELIABLE or BEST_EFFORT.\n\n";
      print_help();
      std::exit(1);
    }
    ...
    if (qos_source_pub.size() != topic_names_pub.size() ||
      ...
      std::cout << "Error: per-endpoint QoS source options must match topic "
                   "entries.\n\n";
      print_help();
      std::exit(1);
    }
...
#undef FIND_MESSAGE_TYPE
      if (!supported) {
        std::cout << "Error: unsupported " << role
