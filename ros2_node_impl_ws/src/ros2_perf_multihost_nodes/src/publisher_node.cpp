  ...
  explicit Publisher(const node_options::Options &options)
    ...
    for (size_t i = 0; i < options.topic_names.size(); ++i) {
      ...
      auto publish_message = [this, topic_name, payload_size,
        ...
        if ((time_stamp.seconds() - start_time_[topic_name].seconds()) >=
            eval_time) {
        ...
        message_->header.stamp.sec = static_cast<int32_t>(
            time_stamp.seconds() - start_time_[topic_name].seconds());
        message_->header.stamp.nanosec = static_cast<uint32_t>(
            (time_stamp.nanoseconds() - start_time_[topic_name].nanoseconds()) %
            1000000000);
        ...
        oss << std::dec << "Time: " << std::fixed << std::setprecision(9)
            << static_cast<double>(time_stamp.nanoseconds() -
                                   start_time_[topic_name].nanoseconds()) /
                   1e9;
