...
public:
  ...
  : Node(options.node_name)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "Shutdown timer created with duration %d seconds",
      ...
      auto publish_message =
          ...
          self_node = options.node_name]() -> void
        {
          ...
          message_->header.stamp.sec = static_cast<int32_t>(time_stamp.seconds() -
            start_time_[topic_name].seconds());
          message_->header.stamp.nanosec = static_cast<uint32_t>((time_stamp.nanoseconds() -
            start_time_[topic_name].nanoseconds()) % 1000000000);
          ...
          oss << std::dec << "Time: " << std::fixed << std::setprecision(9) <<
            static_cast<double>(time_stamp.nanoseconds() - start_time_[topic_name].nanoseconds()) /
            1e9;
...
private:
  ...
  void record_log(
    const std::string & topic_name, const uint32_t & message_idx,
  ...
  void print_qos_settings(const rclcpp::QoS & qos)
  {
    ...
    std::string reliability = (qos_profile.reliability ==
      rmw_qos_reliability_policy_t::RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT) ? "Best Effort" :
      "Reliable";
    std::string durability = (qos_profile.durability ==
      rmw_qos_durability_policy_t::RMW_QOS_POLICY_DURABILITY_TRANSIENT_LOCAL) ? "Transient Local" :
      "Volatile";
    std::string history = (qos_profile.history ==
      rmw_qos_history_policy_t::RMW_QOS_POLICY_HISTORY_KEEP_LAST) ? "Keep Last" : "Keep All";
