...
def generate_dockerfiles(json_content, file_path):
    ...
    if rmw_zenoh_flag:
        ...
        zenoh_router_bridge_command = f". /root/performance_ws/install/setup.sh &&  export ZENOH_ROUTER_CONFIG_URI={
            new_config_path} && ros2 run rmw_zenoh_cpp rmw_zenohd"
        ...
        zenoh_router_config_path = os.path.join(
            host_dir, "multihost_config.json5")
        ...
        for index, node in enumerate(nodes):
            ...
            if index == 0:
                if node.get("publisher"):
                    ...
                    additional_command = f"{zenoh_router_command}. /root/performance_ws/install/setup.sh {zenoh_config_command} && cd /root/performance_ws/install/publisher_node/lib/publisher_node && ./publisher_node_exe --node_name {
                        node_name} --topic_names {topic_names} -s {payload_size} -p {period_ms} --eval_time {eval_time}"
                ...
                if node.get("subscriber"):
                    ...
                    additional_command = f"{zenoh_router_command}. /root/performance_ws/install/setup.sh {
                        zenoh_config_command} && cd /root/performance_ws/install/subscriber_node/lib/subscriber_node && ./subscriber_node --node_name {node_name} --topic_names {topic_names} --eval_time {eval_time}"
                    base_command += additional_command
                ...
                if node.get("intermediate"):
                    ...
                    additional_command = f"{zenoh_router_command}. /root/performance_ws/install/setup.sh {zenoh_config_command} && cd /root/performance_ws/install/intermediate_node/lib/intermediate_node && ./intermediate_node --node_name {
                        node_name} --topic_names_pub {topic_names_pub} --topic_names_sub {topic_names_sub} -s {payload_size} -p {period_ms} --eval_time {eval_time}"
                    base_command += additional_command
            ...
            if node.get("publisher"):
                ...
                additional_command = f" & . /root/performance_ws/install/setup.sh {zenoh_config_command} && cd /root/performance_ws/install/publisher_node/lib/publisher_node && ./publisher_node_exe --node_name {
                    node_name} --topic_names {topic_names} -s {payload_size} -p {period_ms} --eval_time {eval_time}"
                base_command += additional_command
            ...
            if node.get("subscriber"):
                ...
                additional_command = f" & . /root/performance_ws/install/setup.sh {
                    zenoh_config_command} && cd /root/performance_ws/install/subscriber_node/lib/subscriber_node && ./subscriber_node --node_name {node_name} --topic_names {topic_names} --eval_time {eval_time}"
                base_command += additional_command
            ...
            if node.get("intermediate"):
                ...
                additional_command = f" & . /root/performance_ws/install/setup.sh {zenoh_config_command} && cd /root/performance_ws/install/intermediate_node/lib/intermediate_node && ./intermediate_node --node_name {
                    node_name} --topic_names_pub {topic_names_pub} --topic_names_sub {topic_names_sub} -s {payload_size} -p {period_ms} --eval_time {eval_time}"
                base_command += additional_command
