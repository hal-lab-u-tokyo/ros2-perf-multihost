...
def _run_script_common_prefix(
    ...
    lines.extend(
        [
            ...
            'EXEC_LOGS_HOST_DIR="$RUN_RESULTS_HOST_DIR/exec_logs/run${RUN_IDX}"',
            'mkdir -p "$EXEC_LOGS_HOST_DIR"',
            (
                f'LOG_DIR="${{LOG_DIR:-{PROJECT_ROOT_IN_CONTAINER}/{PERF_WS_DIR}/${{RUN_DIR_NAME}}/results/${{RUN_TIMESTAMP}}/exec_logs/run${{RUN_IDX}}}}"'
            ),
            "",
            'cd "$PROJECT_ROOT"',
            "",
