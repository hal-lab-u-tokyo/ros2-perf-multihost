...
def _require_non_empty_string(entry, key, context):
...
def _ensure_only_allowed_keys(entry, allowed_keys, context):
...
def _validate_qos_schema(qos):
    ...
    _ensure_only_allowed_keys(
        qos, {"history", "depth", "reliability"}, context)
...
def _validate_publisher_entries(pub_entries, context):
    ...
    for pub_idx, pub in enumerate(pub_entries):
        ...
        _require_non_empty_string(pub, "topic_name", pub_context)
        _require_positive_int(pub, "payload_size", pub_context)
        _require_positive_int(pub, "period_ms", pub_context)


def _validate_subscriber_entries(sub_entries, context):
    ...
    for sub_idx, sub in enumerate(sub_entries):
        ...
        _ensure_only_allowed_keys(sub, {"topic_name"}, sub_context)
        _require_non_empty_string(sub, "topic_name", sub_context)


def _validate_topology_json_schema(json_content):
    """Validate the topology JSON against the schema documented in topology_example/README.md."""
    root_context = "root"
    ...
    for host_idx, host in enumerate(hosts):
        ...
        _ensure_only_allowed_keys(host, {"host_name", "nodes"}, host_context)
        _require_non_empty_string(host, "host_name", host_context)
        ...
        for node_idx, node in enumerate(nodes):
            ...
            _require_non_empty_string(node, "node_name", node_context)
...
if __name__ == "__main__":
    ...
    _validate_topology_json_schema(json_content)
