...
QoS settings are applied at runtime. If the topology JSON defines `qos` as an array,
the generated files use the first QoS case as their default, and the future sweep
runner should pass the current case via `--qos-*` options or `QOS_*` environment
variables.
