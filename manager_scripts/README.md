...
`payload_size` and `period_ms` must be specified in each `nodes[].publishers[]` entry in the topology JSON, and those values are passed directly to benchmark nodes.
