...
| File | Purpose |
|---|---|
| `host{N}_exec_docker.sh` | Wrapper script that launches the host-specific Compose file with automatic UID/GID handling |
| `host{N}_exec_native.sh` | Wrapper script that runs `host{N}.launch.py` directly via `ros2 launch` on the host (native execution mode) |
| `host{N}_compose.yaml` | Host-specific Compose definition for real multi-host deployment |
| `host{N}.launch.py` | ROS 2 launch file executed via `ros2 launch` (used by both Docker and native modes) |
...
Generated `host*_exec_docker.sh`, `host*_exec_native.sh`, and `local_exec.sh` scripts support the runtime options below.
...
`--trial-idx` is available only on `host*_exec_docker.sh`, `host*_exec_native.sh`, and `local_exec.sh`.
...
Each node launched from `host{N}_exec_docker.sh`, `host{N}_exec_native.sh`, or `local_exec.sh` receives a `--log-dir` under `results/YYYY-MM-DD_hh-mm-ss-<rmw>/exec_logs/trial<trial_idx>/` inside the generated topology directory. `results/latest-<rmw>` is updated as a symbolic link to the active run directory.
