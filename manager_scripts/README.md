...
This directory contains scripts for generating and distributing topology-specific execution artifacts, starting REST servers on all Hosts, and operating the Zenoh router and the Fast DDS Discovery Server.
...
| `fastdds_discoveryd_compose.yaml` | Standalone Compose file for starting the Fast DDS Discovery Server container in Docker execution mode (used only when `--fastdds-discovery-server` is set) |
...
`--eval-time` is applied to every launched benchmark node. When running through
`performance_test.py`, the manager passes the full node runtime here
(`warmup-time + eval-time` plus internal padding) and uses `--warmup-time` only
during aggregation.
...
- `zenohd_compose.yaml`
- `fastdds_discoveryd_compose.yaml`
- `metadata.txt`
...
`docker logs`. When `--fastdds-discovery-server` is set, the native `fastdds
discovery` process follows the same convention under `fastdds_discoveryd.log`.
