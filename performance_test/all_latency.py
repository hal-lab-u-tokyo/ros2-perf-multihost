                    ...
                    if sub_topic in pub_topic_list:
                        ...
                        if pub_node_type == "Publisher":
                            pub_logdata_path = os.path.join(
                                logs_folder_path,
                                f"{pub_node_name}_log",
                                f"{sub_topic}_log.txt",
                            )
                        elif pub_node_type == "Intermediate":
                            pub_logdata_path = os.path.join(
                                logs_folder_path,
                                f"{pub_node_name}_log",
                                f"{sub_topic}_pub_log.txt",
                            )
                        ...
                        # この共通集合に入る時間帯が計測対象
                        common_start_time = (
                            int(max(pub_start_time, sub_start_time)) + warmup_ns
                        )
                        ...
                        pub_indices = {
                            item[0]
                            for item in pub_logdata_list
                            if int(item[1]) >= common_start_time
                            and int(item[1]) <= common_end_time
                        }
                        sub_indices = {
                            item[0]
                            for item in sub_logdata_list
                        ...
                        loss_index_count = len(
                            set(pub_indices) - set(sub_indices)
                        ) + len(set(sub_indices) - set(pub_indices))
                        ...
                        for index in common_indices:
                            latency_results.append(
                                (sub_dict[index] - pub_dict[index]) / 1_000_000
                            )
                        # for index in common_indices:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results", type=str, default="./results", help="Base path for results"
    )
