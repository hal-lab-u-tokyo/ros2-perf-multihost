            ...
            for sub_topic in sub_topic_list:
                ...
                for pub_node_info in all_node_info:
                    ...
                    if sub_topic in pub_topic_list:
                        ...
                        pub_logdata_list, rtt_map = get_log(pub_logdata_path, pub_node_type)
                        sub_logdata_list, _ = get_log(sub_logdata_path, sub_node_type)
                        ...
                        for index in common_indices:
                            # 優先: publisher の RTT を使う (存在し, >=0)。単位は us -> ms に変換。
                            if index in rtt_map and rtt_map[index] is not None:
                                val = rtt_map[index]
                                if val >= 0:
                                    latency_results.append(val / 1000.0)
                                else:
                                    # タイムアウト等 -1 はスキップ or 特殊扱い (ここでは -1 を ms として追加しない)
                                    pass
                            else:
                                # フォールバック: sub_timestamp - pub_timestamp (従来挙動)
                                if index in pub_dict and index in sub_dict:
                                    latency_results.append((sub_dict[index] - pub_dict[index]) / 1_000_000)
                                else:
                                    # どちらのデータも揃っていない場合はスキップ
                                    pass
