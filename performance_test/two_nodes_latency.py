...
def check_connect(args):
    ...
    with open(pub_metadata_path, "r") as f:
        ...
        pub_option_list = []
        for topic, payload_size, period_ms in zip(
            pub_topic_list, pub_payload_size_list, pub_period_ms_list
        ):
...
def measure_latency(pub_logdata, sub_logdata, topic_list):
    ...
    for topic in topic_list:
        ...
        los_index_count = len(set(pub_indices) - set(sub_indices)) + len(
